...
PATTERNS = [
    ...
    ("anchor_line_start", r"{@line_start}"),
    ("anchor_line_end", r"{@line_end}"),
    ("anchor_doc_start", r"{@doc_start}"),
    ("anchor_doc_end", r"{@doc_end}"),
...
def _build_script_compiled() -> dict[str, Any]:
    return {
        name: _canon_pipeline(
            parser.compile_script((SCRIPTS / name).read_text("utf-8"))
        )
