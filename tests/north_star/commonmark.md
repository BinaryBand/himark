...
IETF **Author:** Sean Leonard (Penango, Inc.) **Document identifier:** RFC7764-v1-draft **Category:** Informational **Date:** March 2016 **License:** Simplified BSD License (IETF Trust)
...
- [4. Dive into Markdown](#4-dive-into-markdown)
  - [4.1. On Formats](#41-on-formats)
  - [4.2. Markdown Design Philosophy](#42-markdown-design-philosophy)
  - [4.3. Uses of Markdown](#43-uses-of-markdown)
  - [4.4. Uses of Labeling Markdown Content as text/markdown](#44-uses-of-labeling-markdown-content-as-textmarkdown)
  - [4.5. Definitions](#45-definitions)
- [5. Strategies for Preserving Media Type and Parameters](#5-strategies-for-preserving-media-type-and-parameters)
  - [5.1. Map to Filename and Attributes](#51-map-to-filename-and-attributes)
  - [5.2. Store Headers in Adjacent File](#52-store-headers-in-adjacent-file)
  - [5.3. "Arm" Content with MIME Headers](#53-arm-content-with-mime-headers)
  - [5.4. Create a Local Batch Script](#54-create-a-local-batch-script)
  - [5.5. Process the Markdown in Advance](#55-process-the-markdown-in-advance)
  - [5.6. Rely on Context](#56-rely-on-context)
  - [5.7. Specific Strategies](#57-specific-strategies)
- [6. Registration Templates for Common Markdown Syntaxes](#6-registration-templates-for-common-markdown-syntaxes)
  - [6.1. MultiMarkdown](#61-multimarkdown)
  - [6.2. GitHub Flavored Markdown](#62-github-flavored-markdown)
  - [6.3. Pandoc](#63-pandoc)
  - [6.4. Fountain (Fountain.io)](#64-fountain-fountainio)
  - [6.5. CommonMark](#65-commonmark)
  - [6.6. kramdown-rfc2629 (Markdown for RFCs)](#66-kramdown-rfc2629-markdown-for-rfcs)
  - [6.7. rfc7328 (Pandoc2rfc)](#67-rfc7328-pandoc2rfc)
  - [6.8. PHP Markdown Extra](#68-php-markdown-extra)
- [7. Examples for Common Markdown Syntaxes](#7-examples-for-common-markdown-syntaxes)
  - [7.1. MultiMarkdown](#71-multimarkdown)
  - [7.2. GitHub-Flavored Markdown](#72-github-flavored-markdown)
  - [7.3. Pandoc](#73-pandoc)
  - [7.4. Fountain (Fountain.io)](#74-fountain-fountainio)
  - [7.5. CommonMark](#75-commonmark)
  - [7.6. kramdown-rfc2629 (Markdown for RFCs)](#76-kramdown-rfc2629-markdown-for-rfcs)
  - [7.7. rfc7328 (Pandoc2rfc)](#77-rfc7328-pandoc2rfc)
...
- **variant** â€” A Markdown syntax with varying degrees of formal documentation and implementation, identified by a registered variant identifier (see [RFC7763]).
...
```text
informal /---------formatted text----------\\ formal <------v-------------v-------------v-----------------------v---->
plain text informal markup formal markup binary format (Markdown) (HTML, XML, etc.)
```

Figure 1: Degrees of Formality in Data-Storage Formats for Text

### 4.2 Markdown Design Philosophy
...
### 4.4 Uses of Labeling Markdown Content as text/markdown
...
## 5. Strategies for Preserving Media Type and Parameters
...
### 5.1 Map to Filename and Attributes
...
### 5.2 Store Headers in Adjacent File
...
### 5.3 "Arm" Content with MIME Headers
...
### 5.4 Create a Local Batch Script
...
### 5.5 Process the Markdown in Advance
...
## 6. Registration Templates for Common Markdown Syntaxes
...
**Description:** Pandoc is designed for multiple output formats. While pandoc allows the embedding of raw HTML, it discourages it, and provides other, non-HTMLish ways of representing important document elements like definition lists, tables, mathematics, and footnotes.

**Additional Parameters:** extensions: String with an optional starting syntax token, followed by a "+" and "-" delimited list of extension tokens (see Pandoc docs).
...
### 6.4 Fountain (Fountain.io)

**Identifier:** Fountain

**Name:** Fountain

**Description:** Fountain is a simple markup syntax for writing, editing, and sharing screenplays in plain, human-readable text.
...
**Description:** CommonMark is a standard, unambiguous syntax specification for Markdown, along with a suite of comprehensive tests to validate Markdown implementations against this specification.
...
### 6.6 kramdown-rfc2629 (Markdown for RFCs)

**Identifier:** kramdown-rfc2629

**Name:** Markdown for RFCs

**Description:** kramdown is a markdown parser by Thomas Leitner; kramdown-rfc2629 is an additional backend that allows generation of xml2rfc XML markup.

**References:** <https://github.com/cabo/kramdown-rfc2629>

**Contact Information:** (individual) Carsten Bormann <cabo@tzi.org>

### 6.7 rfc7328 (Pandoc2rfc)

**Identifier:** rfc7328

**Name:** Pandoc2rfc

**Description:** Pandoc2rfc allows authors to write in "pandoc" that is then transformed to XML and given to xml2rfc.

**References:** RFC 7328 <https://github.com/miekg/pandoc2rfc>

**Contact Information:** (individual) R. (Miek) Gieben <miek@google.com>

### 6.8 PHP Markdown Extra

**Identifier:** Extra

**Name:** Markdown Extra

**Description:** Markdown Extra is an extension to PHP Markdown implementing additional features such as attribute blocks and footnotes.
...
## 7. Examples for Common Markdown Syntaxes

This section provides examples of the variants in Section 6.
...
### 7.6 kramdown-rfc2629 (Markdown for RFCs)
...
them â€” who may not be in a position to deploy a new protocol
...
When writing [RFC4641], we wrote the XML by hand. This was tedious, even though the `xml2rfc` XML is fairly lightweight. The latest `xml2rfc` (version 2) is available on [PyPI](https://pypi.org/project/xml2rfc/).
...
IANA has registered the syntaxes specified in Section 6 in the "Markdown Variants" registry.
...
- **[MD102b8]** Gruber, J., "Subject: [ANN] Markdown.pl 1.0.2b8", message to the markdown-discuss mailing list, 9 May 2007. <http://six.pairlist.net/pipermail/markdown-discuss/2007-May/000615.html>
...
- **[INETMEME]** Solon, O., "Richard Dawkins on the internet's hijacking of the word 'meme'", June 2013. <http://www.wired.co.uk/news/archive/2013-06/20/richard-dawkins-memes>
