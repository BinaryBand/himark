...
(see `tools/precompiled.compile_script`). These pin the loader's classification,
scope/collision rules, and the invariant that a definition leaves no trace in the
compiled pipeline."""

import pytest

from himark.tools import precompiled as P
from himark.models.exceptions import CompileError


def _run(source: str, text: str) -> str:
    return P.apply(P.compile_script(source), text)
...
def test_definition_leaves_no_trace():
    ...
    defined = P.compile_script('@n = {@d}[1..]\n@n{\\,}@n => "{{$0}}+{{$1}}"\n')
    inlined = P.compile_pipeline(['{@d}[1..]{\\,}{@d}[1..] => "{{$0}}+{{$1}}"'])
