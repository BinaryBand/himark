...
set `HIMARK_ANTLR_PARSER` to a module path (default `himark.parser_antlr`) exposing
    ...
    parse(text: str, macros: dict[str, str] | None = None) -> list[RootNode]
...
`compile_script_with` (the real `precompiled` splitter/macro logic, candidate parser
...
def compile_script_with(source: str, parse_fn: ParseFn) -> list[list]:
    ...
    for item in precompiled.split_statements(source):
        ...
        steps = parse_fn(converted, macros=local)
        if loop and steps:
...
def load_candidate() -> ModuleType | None:
    """The ANTLR-backed parser module, if present. See the contract in the module
    docstring. Absent → the live-differential tests skip (the golden pins still
    run, guarding the existing parser)."""
