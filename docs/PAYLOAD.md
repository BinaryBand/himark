...
| binop | `{"binop": [op, <lhs>, <rhs>]}` | a binary value operator. `op` is the spelling: arithmetic `+ - * / %`, bitwise `& ^ << >>`, or a backtick for or. Computed on the operands' values; the **codec is the RHS alphabet** (else LHS) and the **band is the LHS** (else RHS), then normalize+encode (see [ALGEBRA.md](ALGEBRA.md)). Total: `x/0 = x%0 = 0`. |
| alpha | `{"alpha": "<name>"}` | a named-alphabet reference (a value-0 codec carrier). Appears as the RHS of a `binop` `+` desugared from a `\| name` **cast**: the RHS alphabet becomes the result's codec, and it carries no band, so the recode keeps the LHS domain (lossless). The name resolves to an alphabet declared in `std.hmk` (or a script-local `@name =`); the wire form only names it. |
